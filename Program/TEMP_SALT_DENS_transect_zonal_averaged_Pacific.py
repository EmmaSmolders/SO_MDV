...
def ReadinData(filename, lon_min, lon_max, lat_min, lat_max, layer_avail = False, volume_norm = False):
	...
	temp 		= fh.variables['TEMP'][:, lat_min_index:lat_max_index,:]		#Potential temperature (deg C)
	salt 		= fh.variables['SALT'][:, lat_min_index:lat_max_index,:] * 1000. 	#Salinity (g /kg)
	#dens		= fh.variables['PD'][:, lat_min_index:lat_max_index,:] * 1000.	#Potential density (kg / m3)
	...
	lon_2, depth_grid	=  ConverterField2D(lon, depth_grid)
	lon_2, salt		=  ConverterField3D(lon, salt)
	lon_2, dens		=  ConverterField3D(lon, dens)
	lon, temp		=  ConverterField3D(lon, temp)
	...
	if lon_min < 0 or lon_max > 360:
		...
		lon_2, temp		= PeriodicBoundaries3D(depth, lon, lat, temp, 1800)
		lon_2, salt		= PeriodicBoundaries3D(depth, lon, lat, salt, 1800)
		lon_2, dens		= PeriodicBoundaries3D(depth, lon, lat, dens, 1800)
	...
	salt		= salt[:, :, lon_min_index:lon_max_index]
	dens		= dens[:, :, lon_min_index:lon_max_index]
	
	print(np.shape(salt))
	print(np.shape(lat))
	print(np.shape(lon))
	
	print(np.shape(depth_grid))
	print(np.shape(depth_top))
	...
	if layer_avail	== False:	
		...
		fh = netcdf.Dataset(directory+'Ocean/Volume_90S_10N_150E_295E_Pacific_basin.nc', 'w')
...
#-----------------------------------------------------------------------------------------
lat, depth, volume_norm, grid_y, temp, salt, dens	= ReadinData(files[0], lon_min, lon_max, lat_min, lat_max)
...
for year_i in range(len(time_year)):
	...
	for month_i in range(12):
		...
		lat, depth, volume_norm, grid_y, temp_year[month_i], salt_year[month_i], dens_year[month_i] = ReadinData(filename, lon_min, lon_max, lat_min, lat_max)
...
print('Data is written to file')
fh = netcdf.Dataset(directory+'Ocean/TEMP_SALT_DENS_year_'+str(year_start)+'-'+str(year_end)+'_zonal_averaged_lon_'+str(lon_min)+'-'+str(lon_max)+'_transect_SO_Pacific.nc', 'w')
