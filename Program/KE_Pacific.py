...
def ReadinData(filename, layer_avail = False, volume_norm = False):
	...
	if lon_min < 0 or lon_max > 360:
		...
		lon_2, u_vel		= PeriodicBoundaries3D(lon, lat, u_vel, 1800)
		lon_2, v_vel		= PeriodicBoundaries3D(lon, lat, v_vel, 1800)
...
for year_i in range(len(time_year)):
	...
	fh = netcdf.Dataset(directory+'Ocean/KE_year_'+str(year_start)+'-'+str(year_end)+'_window_'+str(window)+'_volume_integrated_Pacific_finalcode.nc', 'w')
