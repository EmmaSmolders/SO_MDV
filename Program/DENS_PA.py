...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 16 13:16:33 2025

@PAthor: 6008399

Density in convective PA region
...
fh = netcdf.Dataset(directory + 'Ocean/Volume_70_40S_110_60W_PA.nc','r')
...
fh = netcdf.Dataset(directory + 'Ocean/PD_gsw_year_1-600_area_averaged_PA_70_40S_110W_60W.nc','r')
...
fh = netcdf.Dataset(directory_data + 'TEMP_SALT_DENS_year_300-400_zonal_averaged_-110E--60E_PA_SO.nc','r')
...
fh = netcdf.Dataset(directory_data + 'TEMP_SALT_DENS_year_400-500_zonal_averaged_-110E--60E_PA_SO.nc','r')
...
n0_som_1_PA, N2_som_1_PA = compute_N2_from_profile(PD_weighted_1, depth)
n0_som_2_PA, N2_som_2_PA = compute_N2_from_profile(PD_weighted_2, depth)
...
for time_i in range(len(time_all)):
    n0, N2_time[time_i, :] = compute_N2_from_profile(PD_PA_all[time_i,:], depth)
...
CS = ax3.contourf(time_all, depth_PD, (PD_PA_all - np.mean(PD_PA_all[1:100], axis=0)).transpose(), levels=np.linspace(-0.06, .06, 21), extend='both', cmap = 'RdBu_r')
...
ax3.plot(time_mld[window//2:-window//2+1], Moving_average(np.max(MLD_SO_PA, axis=(1,2)), window), color='black', label='maximum MLD')
...
#ax4.plot(N2_som_2_PA - N2_som_1_PA, depth, color='blue', label='SOM2 vs SOM1')
ax4.plot(N2_som_2_PA - N2_som_1_PA, depth, color='red', label='(500-600) vs (1-100)')
...
plt.suptitle('Convection region Pacific (110$^\circ$W - 60$^\circ$W, 70 - 40$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Dens_MLD_N2_PA.pdf')
...
plt.suptitle('Convection region PA (110$^\circ$W - 60$^\circ$W, 70 - 40$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Density_evolving_PA.pdf')
# %%
...
fig.savefig(directory_figures +'Meridional_density_gradients_PA.pdf')
...
fig.savefig(directory_figures +'Salinities_PA.pdf')
...
fig.savefig(directory_figures +'Temperatures_PA.pdf')
# %%
...
fig.savefig(directory_figures +'Buoyancy_frequency_PA.pdf')
# %%
...
plt.suptitle('Convection region PA (35$^\circ$W - 80$^\circ$W, 80 - 50$^\circ$S)', fontsize=16)

plt.tight_layout()
plt.savefig(directory_figures +'DENS_components_PA_OS_HR_POP.pdf')
plt.show()
# %%
...
plt.tight_layout()
plt.savefig(directory_figures +'N2_components_PA_OS_HR_POP.pdf')
plt.show()