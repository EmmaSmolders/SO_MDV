...
Energetics Pacific with final code and from 1 - 600
...
fh = netcdf.Dataset(directory + 'Ocean/KE_year_63-600_window_5_volume_integrated_Pacific_finalcode.nc','r')

time = fh.variables['time'][:] #Starting year of window SOM cycle 1
TKE_Pacific = fh.variables['TKE'][:] #Volume integrated total kinetic energy [J]
MKE_Pacific = fh.variables['MKE'][:] #Volume integrated mean kinetic energy [J]
EKE_Pacific = fh.variables['EKE'][:] #Volume integrated eddy kinetic energy [J]
...
fh = netcdf.Dataset(directory + 'Ocean/Generation_KE_year_63-600_window_5_area_integrated_Pacific.nc','r')

time = fh.variables['time'][:] #Starting year of window SOM cycle 1
gKe_Pacific = fh.variables['gKe'][:] 
gKm_Pacific = fh.variables['gKm'][:]
gKtotal_Pacific = fh.variables['gKtotal'][:]
...
fh = netcdf.Dataset(directory + 'Ocean/Conversion_PE_KE_year_63-600_5_volume_integrated_Pacific_testjuling.nc','r')

time = fh.variables['time'][:] #Starting year of window SOM cycle 1
C_total_Pacific = fh.variables['C_total'][:] #Volume integrated total kinetic energy [J]
C_mean_Pacific = fh.variables['C_mean'][:] #Volume integrated mean kinetic energy [J]
C_eddy_Pacific = fh.variables['C_eddy'][:] #Volume integrated eddy kinetic energy [J]
...
ax1.plot(time_1, APE_Pacific_1/1e18, color='cyan')
ax1.plot(time_2, APE_Pacific_2/1e18, color='cyan')
ax1.plot(time_3, APE_Pacific_3/1e18, color='cyan')
...
ax2.plot(time, TKE_Pacific/1e18, label='K', color='blue')
...
plt.suptitle('Energetics Pacific', fontsize=16)
plt.tight_layout()
plt.savefig(directory_figures +'Energetics_Pacific_1-600.pdf')
plt.show()
...
ax1.plot(time_som_1, norm(TKE_Pacific[0:len(time_som_1)], TKE_Pacific[0:len(time_som_1)]), label='K', color='blue', linewidth=3) #time + 2 because of centered window moving average (and time_1 displays starting year of the window)
ax1.plot(time_som_1, norm(APE_Pacific_1[63:63+len(time_som_1)], APE_Pacific_1[63:63+len(time_som_1)]), label='P', color='cyan', linewidth=3)
ax1.plot(time_som_1, norm(gKm_Pacific[0:len(time_som_1)], gKm_Pacific[0:len(time_som_1)]), label='G(K$_m$)', color='red', linewidth=3)
ax1.plot(time_som_1, norm(C_eddy_Pacific[0:len(time_som_1)], C_eddy_Pacific[0:len(time_som_1)]), label='C(P$_e$, K$_e$)', color='magenta', linewidth=3)
...
ax2.plot(time_som_2, norm(TKE_Pacific[410 - 63:410-63 + len(time_som_2)], TKE_Pacific[0:len(time_som_1)]), label='TKE', color='blue', linewidth=3)
ax2.plot(time_som_2, norm(APE_Pacific_3[20:20 + len(time_som_2)], APE_Pacific_1[63:63+len(time_som_1)]), label='APE', color='cyan', linewidth=3)
ax2.plot(time_som_2, norm(gKm_Pacific[410 - 63:410-63 + len(time_som_2)], gKm_Pacific[0:len(time_som_1)]), label='Wind', color='red', linewidth=3)
ax2.plot(time_som_2, norm(C_eddy_Pacific[410 - 63:410-63 + len(time_som_2)], C_eddy_Pacific[0:len(time_som_1)]), label='PE -> KE', color='magenta', linewidth=3)
...
ax3.plot(time_som_3, norm(TKE_Pacific[500 - 63::], TKE_Pacific[0:len(time_som_1)]), color='blue', linewidth=3)
ax3.plot(time_som_3, norm(APE_Pacific_3[-97::], APE_Pacific_1[63:63+len(time_som_1)]), color='cyan', linewidth=3)
ax3.plot(time_som_3, norm(gKm_Pacific[500 - 63::], gKm_Pacific[0:len(time_som_1)]),  color='red', linewidth=3)
ax3.plot(time_som_3, norm(C_eddy_Pacific[500 - 63::], C_eddy_Pacific[0:len(time_som_1)]), color='magenta', linewidth=3) #OLD ONE!!
...
plt.suptitle('Energetics Pacific', fontsize=22)

plt.tight_layout()
plt.savefig(directory_figures +'Energetics_SOM_Pacific.pdf')
plt.show()
