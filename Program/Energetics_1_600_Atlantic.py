...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  8 17:27:20 2025

@author: 6008399

Energetics Atlantic sector with final code and from 1 - 600
...
fh = netcdf.Dataset(directory_data + 'KE_year_63-600_window_5_volume_integrated_Atlantic_finalcode.nc','r')

time_K = fh.variables['time'][:] #Starting year of window SOM cycle 1
TKE_Atlantic = fh.variables['TKE'][:] #Volume integrated total kinetic energy [J]
MKE_Atlantic = fh.variables['MKE'][:] #Volume integrated mean kinetic energy [J]
EKE_Atlantic = fh.variables['EKE'][:] #Volume integrated eddy kinetic energy [J]
...
fh = netcdf.Dataset(directory_data + 'Generation_KE_year_63-600_window_5_area_integrated_Atlantic.nc','r')

time_wind = fh.variables['time'][:] #Starting year of window SOM cycle 1
gKe_Atlantic = fh.variables['gKe'][:] 
gKm_Atlantic = fh.variables['gKm'][:]
gKtotal_Atlantic = fh.variables['gKtotal'][:]
...
fh = netcdf.Dataset(directory_data + 'Conversion_PE_KE_year_63-600_5_volume_integrated_Atlantic_testjuling.nc','r')

time_C = fh.variables['time'][:] #Starting year of window SOM cycle 1
C_total_Atlantic = fh.variables['C_total'][:] #Volume integrated total kinetic energy [J]
C_mean_Atlantic = fh.variables['C_mean'][:] #Volume integrated mean kinetic energy [J]
C_eddy_Atlantic = fh.variables['C_eddy'][:] #Volume integrated eddy kinetic energy [J]
...
plt.plot(gKm_Atlantic)
#%% Total energetics

fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(8, 10))

ax1.plot(time_1, APE_Atlantic_1/1e18, color='cyan')
ax1.plot(time_2, APE_Atlantic_2/1e18, color='cyan')
ax1.plot(time_3, APE_Atlantic_3/1e18, color='cyan')
...
ax2.plot(time_K, TKE_Atlantic/1e18, label='K', color='blue')
...
ax2_twin = ax2.twinx()
ax2_twin.plot(time_K, EKE_Atlantic/1e18, label='K', color='grey')
ax2_twin.set_ylabel('K$_e$ [EJ]', color='grey')
ax2_twin.tick_params(axis='y', labelcolor='grey')
ax2.set_xlim(0,600)

ax3.plot(time_wind, gKm_Atlantic/1e12, color='red')
ax3.set_title('c) Mean wind energy input (G(K$_m$))')
ax3.set_ylabel('G(K$_m$) [TW]')
ax3.set_xlim(0,600)
ax3.grid(True)

ax4.plot(time_C, C_eddy_Atlantic/1e12, color='magenta')
...
plt.suptitle('Energetics Atlantic', fontsize=16)
plt.tight_layout()
plt.savefig(directory_figures +'Energetics_Atlantic_1-600.pdf')
plt.show()
...
ax1.plot(time_som_1, norm(TKE_Atlantic[0:len(time_som_1)], TKE_Atlantic[0:len(time_som_1)]), label='K', color='blue', linewidth=3) #time + 2 because of centered window moving average (and time_1 displays starting year of the window)
ax1.plot(time_som_1, norm(APE_Atlantic_1[63:63+len(time_som_1)], APE_Atlantic_1[63:63+len(time_som_1)]), label='P', color='cyan', linewidth=3)
ax1.plot(time_som_1, norm(gKm_Atlantic[0:len(time_som_1)], gKm_Atlantic[0:len(time_som_1)]), label='G(K$_m$)', color='red', linewidth=3)
ax1.plot(time_som_1, norm(C_eddy_Atlantic[0:len(time_som_1)], C_eddy_Atlantic[0:len(time_som_1)]), label='C(P$_e$, K$_e$)', color='magenta', linewidth=3)
...
ax2.plot(time_som_2, norm(TKE_Atlantic[410 - 63:410-63 + len(time_som_2)], TKE_Atlantic[0:len(time_som_1)]), label='TKE', color='blue', linewidth=3)
ax2.plot(time_som_2, norm(APE_Atlantic_3[20:20 + len(time_som_2)], APE_Atlantic_1[63:63+len(time_som_1)]), label='APE', color='cyan', linewidth=3)
ax2.plot(time_som_2, norm(gKm_Atlantic[410 - 63:410-63 + len(time_som_2)], gKm_Atlantic[0:len(time_som_1)]), label='Wind', color='red', linewidth=3)
ax2.plot(time_som_2, norm(C_eddy_Atlantic[410 - 63:410-63 + len(time_som_2)], C_eddy_Atlantic[0:len(time_som_1)]), label='PE -> KE', color='magenta', linewidth=3)
...
ax3.plot(time_som_3, norm(TKE_Atlantic[500 - 63::], TKE_Atlantic[0:len(time_som_1)]), color='blue', linewidth=3)
ax3.plot(time_som_3, norm(APE_Atlantic_3[-97::], APE_Atlantic_1[63:63+len(time_som_1)]), color='cyan', linewidth=3)
ax3.plot(time_som_3, norm(gKm_Atlantic[500 - 63::], gKm_Atlantic[0:len(time_som_1)]),  color='red', linewidth=3)
ax3.plot(time_som_3, norm(C_eddy_Atlantic[500 - 63::], C_eddy_Atlantic[0:len(time_som_1)]), color='magenta', linewidth=3) #OLD ONE!!
...
plt.suptitle('Energetics Atlantic', fontsize=22)

plt.tight_layout()
plt.savefig(directory_figures +'Energetics_SOM_Atlantic.pdf')
plt.show()
