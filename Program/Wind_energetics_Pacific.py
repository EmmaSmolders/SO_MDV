#Program determines wind energy input for Pacific region
...
def ReadinData(filename):
	...
	lon_2, tau_y 	= ConverterField2D(lon, tau_y)
	lon_2, u_vel	= ConverterField2D(lon, u_vel)
	lon_2, v_vel	= ConverterField2D(lon, v_vel)
	lon, tau_x	= ConverterField2D(lon, tau_x)
	
	print(lon)

	if lon_min < 0 or lon_max > 360:
		#If needed, extend the field to add negative longitudes and beyond 360E
		lon_2, u_vel	= PeriodicBoundaries2D(lon, lat, u_vel, 1800)
		lon_2, v_vel	= PeriodicBoundaries2D(lon, lat, v_vel, 1800)
		lon_2, tau_x	= PeriodicBoundaries2D(lon, lat, tau_x, 1800)
		lon_2, tau_y	= PeriodicBoundaries2D(lon, lat, tau_y, 1800)
	...
	tau_x		= tau_x[:, lon_min_index:lon_max_index]
	tau_y		= tau_y[:, lon_min_index:lon_max_index]
...
fh = netcdf.Dataset(directory+'Ocean/Generation_KE_year_'+str(year_start)+'-'+str(year_end)+'_window_'+str(window)+'_area_integrated_Pacific.nc', 'w')
