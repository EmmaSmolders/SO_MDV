...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 16 12:55:09 2025

@author: 6008399

Density in AU region
...
fh = netcdf.Dataset(directory + 'Ocean/PD_gsw_year_1-600_area_averaged_AU_70_40S_80_150E.nc','r')
...
fh = netcdf.Dataset(directory + 'Ocean/Volume_70_40S_80_150E_AU.nc','r')
...
fh = netcdf.Dataset(directory_data + 'TEMP_SALT_DENS_year_300-400_zonal_averaged_80E-150E_AU_SO.nc','r')
...
fh = netcdf.Dataset(directory_data + 'TEMP_SALT_DENS_year_400-500_zonal_averaged_80E-150E_AU_SO.nc','r')
...
n0_som_1_AU, N2_som_1_AU = compute_N2_from_profile(PD_weighted_1, depth)
n0_som_2_AU, N2_som_2_AU = compute_N2_from_profile(PD_weighted_2, depth)
...
plt.figure()
plt.plot(N2_som_1_AU, depth)
plt.plot(N2, depth)
plt.plot(np.mean(N2_lat_1, axis=1), depth)
plt.plot(np.mean(N2_lat_2, axis=1), depth)
plt.ylim(2000, 0)

N2_time = ma.masked_all((len(time_all), len(depth)))
for time_i in range(len(time_all)):
    n0, N2_time[time_i, :] = compute_N2_from_profile(PD_AU_all[time_i,:], depth)
...
CS = ax3.contourf(time_all, depth_PD, (PD_AU_all - np.mean(PD_AU_all[1:100], axis=0)).transpose(), levels=np.linspace(-0.06, .06, 21), extend='both', cmap = 'RdBu_r')
...
ax3.plot(time_mld[window//2:-window//2+1], Moving_average(MLD_max_AU, window), color='black', label='maximum MLD')
...
#ax4.plot(N2_som_2_AU - N2_som_1_AU, depth, color='blue', label='SOM2 vs SOM1')
ax4.plot(N2_som_2_AU - N2_som_1_AU, depth, color='red', label='(500-600) vs (1-100)')
...
plt.suptitle('Convection region AU (80$^\circ$E- 150$^\circ$E, 70 - 40$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Dens_MLD_N2_AU.pdf')
...
ax1.vlines(x=-40, ymin=2000, ymax=0, color='grey')
...
ax2.vlines(x=-40, ymin=2000, ymax=0, color='grey')
...
plt.suptitle('Convection region AU (80$^\circ$E - 150$^\circ$W, 70 - 40$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Density_evolving_AU.pdf')
# %%
...
fig.savefig(directory_figures +'Meridional_density_gradients_AU.pdf')
# %%
# %%
...
fig.savefig(directory_figures +'Salinities_AU.pdf')
...
fig.savefig(directory_figures +'Temperatures_AU.pdf')
...
fig.savefig(directory_figures +'Buoyancy_frequency_AU.pdf')
...
plt.suptitle('Convection region AU (35$^\circ$W - 80$^\circ$W, 80 - 50$^\circ$S)', fontsize=16)

plt.tight_layout()
plt.savefig(directory_figures +'DENS_components_AU_OS_HR_POP.pdf')
plt.show()
# %%
...
plt.tight_layout()
plt.savefig(directory_figures +'N2_components_AU_OS_HR_POP.pdf')
plt.show()
