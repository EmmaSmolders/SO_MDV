...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 15 15:17:52 2025

@author: 6008399

Density in NZ region
...
fh = netcdf.Dataset(directory + 'Ocean/Volume_70_60S_150_190E_NZ.nc','r')
...
fh = netcdf.Dataset(directory_data + 'PD_gsw_year_1-600_area_averaged_NZ_70_62S_160E_190E.nc','r')
...
fh = netcdf.Dataset(directory_data + 'TEMP_SALT_DENS_year_300-400_zonal_averaged_150E-190E_NZ_SO.nc','r')
...
plt.figure()
plt.plot(np.mean(PD_NZ_1, axis=0), depth)
plt.plot(np.mean(dens_1, axis=1), depth, label='before')

plt.plot(np.mean(PD_NZ_2, axis=0), depth)
...
n0_som_1_NZ, N2_som_1_NZ = compute_N2_from_profile(PD_weighted_1, depth)
n0_som_2_NZ, N2_som_2_NZ = compute_N2_from_profile(PD_weighted_2, depth)
...
CS = ax3.contourf(time_all, depth_PD, (PD_NZ_all - np.mean(PD_NZ_all[1:100], axis=0)).transpose(), levels=np.linspace(-0.06, .06, 21), extend='both', cmap = 'RdBu_r')
...
ax3.set_title('c) MLD maximum and PD anomaly in NZ region', fontsize=14)
...
ax4.plot(N2_som_2_NZ - N2_som_1_NZ, depth, color='red', label='(500-600) vs (1-100)')
ax4.vlines(x=0, ymin=5000, ymax=0, color='black', linestyle = '--')
ax4.set_title('d) Area-averaged N$^2$ difference in NZ region', fontsize=14)
...
plt.suptitle('Convection region NZ (150$^\circ$E - 170$^\circ$W, 70 - 60$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Dens_MLD_N2_NZ.pdf')
...
ax1.set_xlim(-80,-30)
ax1.set_ylim(2000, 0)
#ax1.contour(lat, depth, dens_1, levels = [dens_1[depth_idx0, lat_idx]], colors = 'black')
#ax1.contour(lat, depth, dens_2, levels = [dens_2[depth_idx0, lat_idx]], linestyles = '--', colors='black')
cs = ax1.contour(lat, depth, dens_1, levels = [1027.0], colors = 'black')
ax1.clabel(cs, inline=True, manual=[(-60, 1000)], fontsize=10, fmt='%.1f')
#ax1.contour(lat, depth, dens_300, levels = [1027.0], linestyles = '--', colors='black')
...
ax1.clabel(cs, inline=True, manual=[(-60, 1000)], fontsize=10, fmt='%.1f')
#ax1.contour(lat, depth, dens_300, levels = [1027.7], linestyles = '--', colors='black')
#ax1.set_xticklabels(['80', '70', '60', '50', '40', '30', '20', '10'])
ax1.set_ylabel('Depth [m]', fontsize=12)
ax1.set_xlabel('Latitude [$^\circ$S]', fontsize=12)
ax1.vlines(x=-70, ymin=2000, ymax=0, color='grey')
ax1.vlines(x=-60, ymin=2000, ymax=0, color='grey')
...
ax2.set_ylabel('Depth [m]', fontsize=12)
ax2.set_xlabel('Latitude [$^\circ$S]', fontsize=12)
ax2.vlines(x=-70, ymin=2000, ymax=0, color='grey')
ax2.vlines(x=-60, ymin=2000, ymax=0, color='grey')
...
ax3.set_xlim(-80,-30)
ax3.set_ylim(2000, 0)
#ax3.contour(lat, depth, dens_1, levels = [dens_1[depth_idx0, lat_idx]], colors = 'black')
#ax3.contour(lat, depth, dens_2, levels = [dens_2[depth_idx0, lat_idx]], linestyles = '--', colors='black')
...
ax3.vlines(x=-60, ymin=2000, ymax=0, color='grey')
...
ax4.vlines(x=-60, ymin=2000, ymax=0, color='grey')

plt.suptitle('Convection region NZ (150$^\circ$E - 170$^\circ$W, 70 - 60$^\circ$S)', fontsize=16)
plt.tight_layout()

fig.savefig(directory_figures +'Density_evolving_NZ.pdf')
...
fig.savefig(directory_figures +'Meridional_density_gradients_NZ.pdf')
# %%
...
fig.savefig(directory_figures +'Salinities_NZ.pdf')
...
fig.savefig(directory_figures +'Temperatures_NZ.pdf')
...
fig.savefig(directory_figures +'Buoyancy_frequency_NZ.pdf')
...
plt.vlines(x=-70, ymin=2000, ymax=0, color='grey')
plt.vlines(x=-60, ymin=2000, ymax=0, color='grey')
# %%
...
plt.tight_layout()
plt.savefig(directory_figures +'DENS_components_NZ_OS_HR_POP.pdf')
plt.show()
# %%
...
axs[0,0].vlines(x=-70, ymin=2000, ymax=0, color='grey')
axs[0,0].vlines(x=-60, ymin=2000, ymax=0, color='grey')

CS2 = axs[0,1].contourf(lat, depth, dN2_S, levels = np.linspace(-2e-6, 2e-6, 21), extend = 'both', cmap = 'RdBu_r')
axs[0,1].set_xlim(-75,-30)
axs[0,1].set_ylim(2000, 0)
axs[0,1].contour(lat, depth, dens_1, levels = [1027.1], colors = 'black')
axs[0,1].contour(lat, depth, dens_500, levels = [1027.1], linestyles = '--', colors='black')
axs[0,1].contour(lat, depth, dens_1, levels = [1027.5], colors = 'black')
axs[0,1].contour(lat, depth, dens_500, levels = [1027.5], linestyles = '--', colors='black')
axs[0,1].contour(lat, depth, dens_1, levels = [1027.7], colors = 'black')
axs[0,1].contour(lat, depth, dens_500, levels = [1027.7], linestyles = '--', colors='black')
#axs[0,1].set_xticklabels(['80', '70', '60', '50', '40', '30', '20', '10'])
cbar	= colorbar(CS2, ticks = np.arange(-1e-5, 1.01e-5, 0.5e-5))
#cbar.set_label('Salinity difference [g/kg]', fontsize = 11)
axs[0,1].set_title('b) Salinity-driven N$^2$ change', fontsize=14)
axs[0,1].vlines(x=-70, ymin=2000, ymax=0, color='grey')
axs[0,1].vlines(x=-60, ymin=2000, ymax=0, color='grey')
...
axs[1,0].contour(lat, depth, dens_1, levels = [1027.1], colors = 'black')
axs[1,0].contour(lat, depth, dens_500, levels = [1027.1], linestyles = '--', colors='black')
axs[1,0].contour(lat, depth, dens_1, levels = [1027.5], colors = 'black')
axs[1,0].contour(lat, depth, dens_500, levels = [1027.5], linestyles = '--', colors='black')
axs[1,0].contour(lat, depth, dens_1, levels = [1027.7], colors = 'black')
axs[1,0].contour(lat, depth, dens_500, levels = [1027.7], linestyles = '--', colors='black')
#axs[1,0].set_xticklabels(['80', '70', '60', '50', '40', '30', '20', '10'])
axs[1,0].set_ylabel('Depth [m]', fontsize=12)
axs[1,0].set_xlabel('Latitude [$^\circ$S]', fontsize=12)
axs[1,0].vlines(x=-70, ymin=2000, ymax=0, color='grey')
axs[1,0].vlines(x=-60, ymin=2000, ymax=0, color='grey')
...
axs[1,1].set_ylabel('Depth [m]', fontsize=12)
axs[1,1].set_xlabel('Latitude [$^\circ$S]', fontsize=12)
axs[1,1].vlines(x=-70, ymin=2000, ymax=0, color='grey')
axs[1,1].vlines(x=-60, ymin=2000, ymax=0, color='grey')

plt.suptitle('Convection region NZ (150$^\circ$E - 170$^\circ$W, 70 - 60$^\circ$S)', fontsize=16)

plt.tight_layout()
plt.savefig(directory_figures +'N2_components_NZ_OS_HR_POP.pdf')
plt.show()
# %%
